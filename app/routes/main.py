...
def index():
    ...
    hoteles_destacados = Hotel.query.filter_by(estado='activo').limit(6).all()
    return render_template('index.html', hoteles=hoteles_destacados)
...
def buscar():
    ...
    # Solo habitaciones y hoteles activos
    query = query.filter(
